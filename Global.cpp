...
void CGlobal::Load()
{
	...
	this->m_iRefreshInterval = AfxGetApp()->GetProfileInt("ODBC","RefreshInterval",0);
...
void CGlobal::Save()
{
	...
	AfxGetApp()->WriteProfileInt("ODBC","RefreshInterval",this->m_iRefreshInterval);
...
void CGlobal::Connect()
{
	if( !m_oDSN.IsEmpty() )
	{
		try
		{
			...
			//::AfxMessageBox(m_oDSN);
			m_poQuoteDB->OpenEx( m_oDSN, CDatabase::noOdbcDialog );
		}
