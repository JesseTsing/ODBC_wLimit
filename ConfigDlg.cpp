...
void CConfigDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX,IDC_QryInterval,g_oData.m_iRefreshInterval);
...
void CConfigDlg::FillTableList()
{
	if( g_oData.m_poQuoteDB && g_oData.m_poQuoteDB->IsOpen() )
	{
		...
		retcode = SQLBindCol(hstmt, 1, SQL_C_CHAR, tabQualifier, 255,
							 (SQLLEN *)&lenTabQualifier);
		retcode = SQLBindCol(hstmt, 2, SQL_C_CHAR, tabOwner, 255, (SQLLEN *)&lenTabOwner);
		retcode = SQLBindCol(hstmt, 3, SQL_C_CHAR, tabName, 255, (SQLLEN *)&lenTableName);
		retcode = SQLBindCol(hstmt, 4, SQL_C_CHAR, tabType, 255, (SQLLEN *)&lenTableType);
		retcode = SQLBindCol(hstmt, 5, SQL_C_CHAR, remarks, 255, (SQLLEN *)&lenRemarks);
...
void CConfigDlg::OnOK() 
{
	...
	if( g_oData.m_iRefreshInterval < 1)
	{
		::AfxMessageBox("Refresh Interval Reset to 0!!!!");
		g_oData.m_iRefreshInterval = 0;
	}
	else
	{	
		CString notifystr;
		notifystr.Format("Refresh Interval: %d", g_oData.m_iRefreshInterval);
